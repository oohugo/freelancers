  ...
  context 'validations' do
    context 'present' do
      ...
      it 'description must be present' do
        proposal.valid?
        expect(proposal.errors.full_messages_for(
                 :description
               )).to include('Descrição da proposta não pode ficar em branco')
      end
      ...
      it 'hours per week must be present' do
        ...
        expect(proposal.errors.full_messages_for(
                 :hours_per_week
               )).to include('Horas disponíveis por semana não pode ficar em branco')
      end
      it 'date close must be present' do
        proposal.valid?
        expect(proposal.errors.full_messages_for(
                 :date_close
               )).to include('Expectativa de conclusão não pode ficar em branco')
      end
    end
    context 'greater than zero' do
      it 'hours per week must be grater than zero' do
        ...
        expect(proposal.errors.full_messages_for(
                 :hours_per_week
               )).to include('Horas disponíveis por semana deve ser maior que 0')
      end
    ...
    it 'date close must be less than deadline of project' do
      ...
      expect(proposal.errors.full_messages_for(
               :date_close
             )).to include('Expectativa de conclusão deve ser mais cedo que a data limite do projeto')
    end
  end
end
